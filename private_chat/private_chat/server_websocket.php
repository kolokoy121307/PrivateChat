        ...
        while(socket_recv($changed_socket, $buf, 1024, 0) >= 1)
        {
			...
			if ( !empty($user_name) && !empty($user_message) ) {
				if ( array_key_exists($to_id, $clients) ) {
					//Send to Destination
					$response_text = mask(json_encode(array('name'=>$user_name, 'message'=>$user_message, 'timemsg'=>$timemsg, 'user_image' => $user_image)));
					...
					//Send to Sender
					$response_text = mask(json_encode(array('name'=>'', 'message'=>$to_name.' is offline', 'timemsg'=>$timemsg, 'user_image' => NULL)));
					send_private_message($clients[$user_id], $response_text); 
				}
			}
			break 2;
